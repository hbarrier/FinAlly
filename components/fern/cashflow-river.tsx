...
export function CashflowRiver({ income, expense, days = 30 }: CashflowRiverProps) {
  const gradId = useId()
  const gradIn = `river-in-${gradId}`
  const gradOut = `river-out-${gradId}`

  const { line, area, w, h, padLeft, padRight, zeroY } = useMemo(() => {
    const w = 520, h = 200
    ...
    return {
      line: { i: line('i'), e: line('e') },
      area: { i: area('i'), e: area('e') },
      w, h, padLeft, padRight, zeroY,
    }
  }, [income, expense, days])
  ...
  return (
    ...
    >
      <defs>
        ...
        <linearGradient id={gradOut} x1="0" x2="0" y1="1" y2="0">
      ...
      <path d={area.i} fill={`url(#${gradIn})`} />
      <path d={line.i} stroke="var(--sage)" strokeWidth="2" fill="none" />

      <path d={area.e} fill={`url(#${gradOut})`} />
